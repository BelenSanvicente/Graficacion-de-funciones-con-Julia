...
# ╔═╡ da4c9216-7064-47e9-a700-39a253354751
@bind d Slider(0.1:0.05:1, default = 0.5)

# ╔═╡ 3164a6e9-d2c8-4feb-9a8e-bade04f13d8c
begin
	plot(sin,-π:d:π, title = "Funciones trigonométricas", xlabel = "x", ylabel = "\$\\sin(x)\$", color = "darkorange", label = "seno")
	plot!(cos,-π:d:π, title = "Funciones trigonométricas", xlabel = "x", ylabel = "\$\\sin(x)\$", color = "pink", label = "coseno")
end
...
# ╔═╡ cb810f88-55da-466c-915a-611c594d74bc
begin
h = 0
θ = π/4
r = 10
...
A=(r/g)*(sin(θ) + sqrt((sin(θ))^2+2*((g*h)/(r^2))))

xm=((r^2/g)*(sin(θ) + sqrt((sin(θ))^2)+2*((g*h)/(r^2))))
ym=((r^2)*sin(θ)^2)/(2*g)+h

function R(t)
     return (h+ry*(t/rx)-(1/2)*g*(t/rx)^2)
end
...
# ╔═╡ c828722d-2d6d-400e-94a5-17db358b23bb
# Ejercicio Crea una función que tome parámetros h, r, θ y t, y haga lo descrito en el Ejercicio anterior.

# ╔═╡ 4c5dd707-8974-401b-8f03-9090e7c75ae3
function P(t)
	return q =+ (h+ry*(t/rx)-(1/2)*g*(t/rx)^2)
end
@gif for t in (0:0.01:A)
	plot(R,rx*(0:0.01:t), xaxis = (0,xm), yaxis = (0,ym), style = (:dot),
	color = "pink", lj=(1),legend=false), title = "Tiro Parabólico", xlabel=L"x",ylabel=L"y")
	scatter!(P, rx*t, rx*t, xaxis = (0,xm), yaxis=(0,ym), color="pink", ms=(10), legend=false)
    end
end 

# ╔═╡ b68a2ed9-dbef-45c1-98aa-4e84ed8b75a6
# Ejercicio Crea una animación de cómo la superficie obtenida de la función h(x,y) =cos(x)+ sen(y) se desplaza hacia el eje Y.

# ╔═╡ 7bbcba0a-fa33-456d-816b-7703902f1ce3
begin
f(x,y) = cos(x) +sin(y)
i = 3
ymax = i * 2π

@gif for i in (0:0.01:3);
	(surface((0:0.1:2π),(0:0.1:2*π*i), f,yaxis = (0,ymax), zaxis = -3,3), 
	backround = "white", color = "blue"))
	end
end
